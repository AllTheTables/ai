...
export default createTransformer((fileInfo, api, options, context) => {
  ...
  // Track which identifiers were imported from '@zenning/ai' and should be renamed
  const importedFromAi = new Set<string>();

  // Find import declarations from '@zenning/ai' and collect the imported names
  ...
  // Only rename identifiers that were imported from '@zenning/ai'
  if (importedFromAi.size > 0) {
    root.find(j.Identifier).forEach(identifierPath => {
      const node = identifierPath.node;

      // Only rename if this identifier was imported from '@zenning/ai'
      if (
